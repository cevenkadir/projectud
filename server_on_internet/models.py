...
class Muon(models.Model):
    ...
    detection_time = models.DateTimeField(
        'Detection date', default=timezone.now())
    ...
    def process_muon(self):
        ...
        if len(cnts) == 1:
            ...
            creation_time = dt.datetime.strptime(os.path.basename(
                self.image.name), 'muon_%Y-%m-%d_%H%M%S.%f.jpg')
            ...
            cropped_img = img[coordinates_ys[0]:coordinates_ys[1],
                              coordinates_xs[0]:coordinates_xs[1]]
            cv2.imwrite(self.image.path, cropped_img,
